...
[Feature]
public static class ListCharactersForPlayer
{
    [UsedImplicitly]
    internal static void ConfigureEndpoints(IEndpointRouteBuilder endpoints)
    {
        endpoints.MapGet("character", Execute)
            .WithTags(EndpointTag.Role.Player, EndpointTag.Resource.Character)
            .WithName(nameof(ListCharactersForPlayer))
            .RequirePermissionAny(Permission.CharacterReadOwn);
    }
